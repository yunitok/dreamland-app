...
**Analytics de Comensales** es una sección dentro del módulo GastroLab que permite a perfiles ejecutivos analizar la evolución de comensales por restaurante durante los últimos 3 años, con gráficos interactivos, KPIs comparativos y filtros dinámicos.
...
| **Dominio** | `src/modules/gastrolab/domain/cover-sync/` | Tipos, agregación, orquestación de sync |
| **Persistencia** | `prisma/schema.prisma` | Modelos CoverSnapshot, CoverSyncLog |
| **Acciones** | `src/modules/gastrolab/actions/cover-analytics.ts` | Server actions con queries Prisma |
| **UI** | `src/app/[locale]/(dashboard)/gastrolab/analytics/` | Page + 8 componentes cliente |
...
**Archivo**: `src/modules/gastrolab/domain/cover-sync/cover-aggregator.ts`
...
**Archivo**: `src/modules/gastrolab/domain/cover-sync/sync-orchestrator.ts`
...
**Archivo**: `src/modules/gastrolab/actions/cover-analytics.ts`

Todas las actions:
- Son `"use server"`
- Ejecutan `await requirePermission("gastrolab", "read")` como primera línea
...
├── requirePermission("gastrolab", "read")
├── getRestaurantLocations() → locations
├── Header (título, link back a /gastrolab)
...
```
src/
├── app/[locale]/(dashboard)/gastrolab/analytics/
...
- **Lectura del dashboard**: `requirePermission("gastrolab", "read")`
