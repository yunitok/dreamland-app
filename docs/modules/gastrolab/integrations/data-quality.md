...
GastroLab incluye herramientas de auditoría para verificar la calidad e integridad de los datos importados desde GStock. Estas herramientas son accesibles desde la UI de **GastroLab > Settings** y via scripts CLI.
...
| | |
|---|---|
| **Archivo** | `src/modules/gastrolab/domain/data-quality/audit-core.ts` |
...
Todos los archivos de soporte se encuentran en `src/modules/gastrolab/domain/data-quality/`.
...
| | |
|---|---|
| **Archivo** | `src/modules/gastrolab/domain/gstock-audit/healthcheck.ts` |
| **Tipo** | Función asíncrona server-side |
...
| | |
|---|---|
| **Archivo** | `src/modules/gastrolab/domain/gstock-sync/allergen-report.ts` |
| **Tipo** | Función asíncrona server-side |
...
|--------|-------------------|-------|
| UI (GastroLab > Settings) | `gastrolab:manage` | Server actions con RBAC |
| Scripts CLI | Acceso directo al servidor | Variables de entorno requeridas |

Las herramientas de auditoría **no** están expuestas en la pantalla de procesos admin (`/admin/processes`). Son exclusivas de la sección de GastroLab.
