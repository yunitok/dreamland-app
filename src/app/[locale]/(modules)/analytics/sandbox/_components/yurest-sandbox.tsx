...
import { YUREST_ENDPOINT_GROUPS, type YurestEndpoint } from "@/lib/yurest"
...
export function YurestSandbox() {
  const t = useTranslations("analytics.sandbox")
  const [selectedEndpoint, setSelectedEndpoint] = useState<YurestEndpoint | null>(null)
  const [result, setResult] = useState<ApiResult | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const executeRequest = useCallback(async (endpoint: YurestEndpoint) => {
    setLoading(true)
    setError(null)
    setResult(null)

    try {
      const response = await fetch(`/api/gastrolab/yurest?endpoint=${encodeURIComponent(endpoint.path)}`)
  ...
  const handleSelect = (endpoint: YurestEndpoint) => {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
      ...
      <Card className="lg:h-[calc(100vh-180px)] lg:overflow-auto">
        ...
        <CardContent className="space-y-4 pt-0">
          {YUREST_ENDPOINT_GROUPS.map((group) => (
            <div key={group.label}>
...
function EndpointInfo({ endpoint }: { endpoint: YurestEndpoint }) {
  return (
    <div className="space-y-4">
      ...
      {endpoint.gastrolabMapping && (
        <div className="p-3 rounded-lg bg-primary/5 border border-primary/10">
          <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1.5">
            <ArrowRight className="h-3 w-3" />
            Mapeo a GastroLab
          </p>
          <code className="text-sm font-mono text-primary">{endpoint.gastrolabMapping}</code>
        </div>
      )}
    </div>
  )
    ...
    <div className="space-y-3">
      {endpoint.gastrolabMapping && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Badge variant="outline" className="text-[10px]">Yurest</Badge>
          <ArrowRight className="h-3 w-3" />
          <Badge variant="secondary" className="text-[10px]">{endpoint.gastrolabMapping}</Badge>
