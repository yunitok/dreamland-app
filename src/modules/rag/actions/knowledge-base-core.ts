// Sin "use server" — intencionado: este archivo es llamado por sync orchestrators
...
import {
  upsertKnowledgeVectorsBatchMultiNS,
  deleteVectorsBySourceMultiNS,
  KBVectorMetadata,
} from "@/lib/pinecone"
import { resolveNamespaces } from "@/modules/rag/domain/domains"
...
export async function bulkImportKBCore(
  ...
  await upsertKnowledgeVectorsBatchMultiNS(vectors, namespaces)
...
export async function syncKBBySourceCore(
  ...
  return bulkImportKBCore(entries.map(e => ({ ...e, source })), true, domains)
}
