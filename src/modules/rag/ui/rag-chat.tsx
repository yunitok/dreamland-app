...
function getUsedTools(parts: unknown[]): Set<string> {
  ...
  for (const part of parts) {
    ...
    if (
      ...
      tools.add(p.type.slice(5))
    }
  }
  return tools
}

// ─── Badge de tool configurable ────────────────────────────────
...
const DEFAULT_TOOL_BADGES: ToolBadgeConfig[] = [
  {
...
export interface RagChatProps {
  /** ID del dominio RAG registrado */
  domain: string
  /** Categorias para el filtro (opcional) */
  categories?: QueryCategory[]
  /** Mostrar badges de tools y fuentes citadas */
  isAdmin?: boolean
  /** Preguntas sugeridas (override del dominio) */
  suggestedQuestions?: string[]
  /** Titulo del header */
  headerTitle?: string
  /** Subtitulo/descripcion */
  headerSubtitle?: string
  /** Badges de tools adicionales del dominio */
  toolBadges?: ToolBadgeConfig[]
  /** Callback de feedback (si no se proporciona, no se muestra feedback) */
  onFeedback?: (userMessage: string, value: 1 | -1) => Promise<{ success: boolean; error?: string }>
}
...
export function RagChat({
  ...
  isAdmin,
  suggestedQuestions = [],
  headerTitle = "Asistente RAG",
  headerSubtitle,
  toolBadges = DEFAULT_TOOL_BADGES,
  onFeedback,
}: RagChatProps) {
  ...
  async function handleFeedback(msgId: string, value: 1 | -1) {
    ...
    const result = await onFeedback(userText, value)
    if (result.success) {
      toast.success(value === 1 ? "Gracias por tu valoracion positiva" : "Gracias por tu feedback")
    } else {
      setFeedbacks(prev => {
        const next = { ...prev }
        delete next[msgId]
    ...
    <div className="flex flex-col h-150 rounded-lg border bg-background">
        ...
        {messages.length === 0 && !isLoading && suggestedQuestions.length > 0 && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground text-center pt-4">
              Haz una pregunta sobre cualquier tema documentado en el sistema.
            </p>
          ...
          return (
            ...
            >
              ...
              <div className={`max-w-[80%] space-y-2 ${msg.role === "user" ? "items-end" : "items-start"} flex flex-col`}>
                ...
                {/* Feedback */}
                {onFeedback && msg.role === "assistant" && textContent && (
      ...
      <div className={`p-4 border-t flex gap-2 items-end transition-opacity ${isLoading ? "opacity-50" : ""}`}>
        <Textarea
          ...
          placeholder={isLoading ? "Esperando respuesta..." : "Escribe tu consulta... (Enter para enviar)"}
          rows={2}
