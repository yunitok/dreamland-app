...
import { publishStagedKBEntries, deleteKBBySource } from "@/modules/rag/actions/knowledge-base"
...
export function KBImportPanel({ domain, categories }: KBImportPanelProps) {
  ...
  async function handleNormalize() {
    ...
    setStaged([])
    try {
      const res = await fetch(`/api/rag/kb/normalize`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: rawText, domain }),
      })
  ...
  async function handleParseFile(anonymize = anonymizeSections) {
    ...
    try {
      ...
      const res = await fetch(`/api/rag/kb/parse-file`, {
        method: "POST",
        body: formData,
      })
      if (!res.ok) {
      ...
      for (const { chunks } of sectionTexts) {
        for (const chunk of chunks) {
          const res = await fetch(`/api/rag/kb/normalize`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ text: chunk, source: "file", domain }),
          })
          if (!res.ok) {
  ...
  async function handlePublish() {
    ...
    try {
      if (replaceMode) {
        try {
          const delResult = await deleteKBBySource(domain, source)
      ...
      for (let i = 0; i < toPublish.length; i += BATCH_SIZE) {
        ...
        try {
          const result = await publishStagedKBEntries(
            domain,
            batch.map(e => ({
              title:      e.title,
              content:    e.content,
              ...
              <TabsContent value="texto">
                <div className="space-y-3 pt-3">
                  ...
                  <Textarea
                    ...
                    placeholder="Pega aquí el texto a normalizar..."
                    rows={12}
