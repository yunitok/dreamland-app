
import React from 'react'

export function FlagES({ className }: { className?: string }) {
  return <svg viewBox="0 0 512 512" className={className} xmlns="http://www.w3.org/2000/svg">
    <mask id="es-mask"><circle cx="256" cy="256" r="256" fill="#fff"/></mask>
    <g mask="url(#es-mask)">
      <path fill="#ffda44" d="m0 128 256-32 256 32v256l-256 32L0 384Z"/>
      <path fill="#eee" d="M196 168q-11 1-15 11l-5-1q-15 1-16 16c-1 15 7 16 16 16q11 0 15-11a16 16 0 0 0 17-4 16 16 0 0 0 17 4 16 16 0 1 0 10-20 16 16 0 0 0-27-5q-4-6-12-6m0 8q8 1 8 8 0 8-8 8-7 0-8-8 1-7 8-8m24 0q8 1 8 8 0 8-8 8-7 0-8-8 1-7 8-8m-44 10 4 1 4 8q-1 7-8 7-9 0-8-8 1-7 8-8m64 0q8 1 8 8 0 8-8 8-7 0-8-7l4-8zm-112 38v80h16v-80zm80 0v40c-26 0-48 14-48 32s22 32 48 32 48-14 48-32v-72zm64 0v80h16v-80z"/>
      <path fill="#ff9811" d="M200 160h16v32h-16z"/>
      <path fill="#d80027" d="M0 0v128h512V0zm208 184c-22 0-40 11-40 24l8 8h64l8-8c0-13-18-24-40-24m-72 8a8 8 0 0 0-8 8v8a8 8 0 1 0 16 0v-8a8 8 0 0 0-8-8m144 0a8 8 0 0 0-8 8v8a8 8 0 1 0 16 0v-8a8 8 0 0 0-8-8m-120 32v24h-38a4 4 0 0 0-4 4 4 4 0 0 0 4 4h38v40a24 24 0 0 0 24 24 24 24 0 0 0 24-24 24 24 0 0 0 24 24 24 24 0 0 0 24-24v-24h-48v-48zm72 8a10 10 0 0 0-10 10v12a10 10 0 1 0 20 0v-12a10 10 0 0 0-10-10m24 16v8h38a4 4 0 0 0 4-4 4 4 0 0 0-4-4zm-134 24a4 4 0 0 0-4 4 4 4 0 0 0 4 4h28a4 4 0 0 0 4-4 4 4 0 0 0-4-4zm144 0a4 4 0 0 0-4 4 4 4 0 0 0 4 4h28a4 4 0 0 0 4-4 4 4 0 0 0-4-4zM0 384v128h512V384z"/>
      <path fill="#ffda44" d="M186 196a6 6 0 0 0-6 6 6 6 0 0 0 6 6 6 6 0 0 0 6-6 6 6 0 0 0-6-6m22 0a6 6 0 0 0-6 6 6 6 0 0 0 6 6 6 6 0 0 0 6-6 6 6 0 0 0-6-6m22 0a6 6 0 0 0-6 6 6 6 0 0 0 6 6 6 6 0 0 0 6-6 6 6 0 0 0-6-6"/><path fill="#ff9811" d="M128 208a8 8 0 1 0 0 16h16a8 8 0 1 0 0-16zm144 0a8 8 0 1 0 0 16h16a8 8 0 1 0 0-16zm-96 8v8h64v-8zm-8 16v8h8v16h-8v8h32v-8h-8v-16h8v-8zm-8 40v24q1 12 9 19v-43zm19 0v47h10v-47zm20 0v43q9-7 9-19v-24zm-71 32a8 8 0 1 0 0 16h16a8 8 0 1 0 0-16zm144 0a8 8 0 1 0 0 16h16a8 8 0 1 0 0-16z"/>
      <path fill="#338af3" d="M208 256a16 16 0 0 0-16 16 16 16 0 0 0 16 16 16 16 0 0 0 16-16 16 16 0 0 0-16-16m-80 64a8 8 0 1 0 0 16h16a8 8 0 1 0 0-16zm144 0a8 8 0 1 0 0 16h16a8 8 0 1 0 0-16z"/>
    </g>
  </svg>
}

export function FlagUK({ className }: { className?: string }) {
  return <svg viewBox="0 0 512 512" className={className} xmlns="http://www.w3.org/2000/svg">
    <mask id="uk-mask"><circle cx="256" cy="256" r="256" fill="#fff"/></mask>
    <g mask="url(#uk-mask)">
      <path fill="#eee" d="m0 0 8 22-8 23v23l32 54-32 54v32l32 48-32 48v32l32 54-32 54v68l22-8 23 8h23l54-32 54 32h32l48-32 48 32h32l54-32 54 32h68l-8-22 8-23v-23l-32-54 32-54v-32l-32-48 32-48v-32l-32-54 32-54V0l-22 8-23-8h-23l-54 32-54-32h-32l-48 32-48-32h-32l-54 32L68 0H0z"/>
      <path fill="#0052b4" d="M336 0v108L444 0Zm176 68L404 176h108zM0 176h108L0 68ZM68 0l108 108V0Zm108 512V404L68 512ZM0 444l108-108H0Zm512-108H404l108 108Zm-68 176L336 404v108z"/>
      <path fill="#d80027" d="M0 0v45l131 131h45L0 0zm208 0v208H0v96h208v208h96V304h208v-96H304V0h-96zm259 0L336 131v45L512 0h-45zM176 336 0 512h45l131-131v-45zm160 0 176 176v-45L381 336h-45z"/>
    </g>
  </svg>
}

export function FlagDE({ className }: { className?: string }) {
  return <svg viewBox="0 0 512 512" className={className} xmlns="http://www.w3.org/2000/svg">
    <mask id="de-mask"><circle cx="256" cy="256" r="256" fill="#fff"/></mask>
    <g mask="url(#de-mask)">
      <path fill="#ffda44" d="m0 345 256.7-25.5L512 345v167H0z"/>
      <path fill="#d80027" d="m0 167 255-23 257 23v178H0z"/>
      <path fill="#333" d="M0 0h512v167H0z"/>
    </g>
  </svg>
}

export function FlagFR({ className }: { className?: string }) {
  return <svg viewBox="0 0 512 512" className={className} xmlns="http://www.w3.org/2000/svg">
    <mask id="fr-mask"><circle cx="256" cy="256" r="256" fill="#fff"/></mask>
    <g mask="url(#fr-mask)">
      <path fill="#eee" d="M167 0h178l25.9 252.3L345 512H167l-29.8-253.4z"/>
      <path fill="#0052b4" d="M0 0h167v512H0z"/>
      <path fill="#d80027" d="M345 0h167v512H345z"/>
    </g>
  </svg>
}

export function FlagIT({ className }: { className?: string }) {
  return <svg viewBox="0 0 512 512" className={className} xmlns="http://www.w3.org/2000/svg">
    <mask id="it-mask"><circle cx="256" cy="256" r="256" fill="#fff"/></mask>
    <g mask="url(#it-mask)">
      <path fill="#eee" d="M167 0h178l25.9 252.3L345 512H167l-29.8-253.4z"/>
      <path fill="#6da544" d="M0 0h167v512H0z"/>
      <path fill="#d80027" d="M345 0h167v512H345z"/>
    </g>
  </svg>
}

export function FlagRU({ className }: { className?: string }) {
  return <svg viewBox="0 0 512 512" className={className} xmlns="http://www.w3.org/2000/svg">
    <mask id="ru-mask"><circle cx="256" cy="256" r="256" fill="#fff"/></mask>
    <g mask="url(#ru-mask)">
      <path fill="#0052b4" d="M512 170v172l-256 32L0 342V170l256-32z"/>
      <path fill="#eee" d="M512 0v170H0V0Z"/>
      <path fill="#d80027" d="M512 342v170H0V342Z"/>
    </g>
  </svg>
}
